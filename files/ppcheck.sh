...
nc -z -w5 192.168.65.137 80
if [[ $? -eq 0 ]]; then
echo "Порт 80 веб-сервера $server на хосте 192.168.65.137 доступен"
else
echo "Порт 80 веб-сервера $server на хосте 192.168.65.137 недоступен"    
exit 1
fi

# Проверка существования файла стртовой страницы
